...
def classify(data_original):
    ...
    # split sets
    data_train = data_shuffled[: int(0.9 * len(data_shuffled))]
    data_test = data_shuffled[int(0.1 * len(data_shuffled)) :]

    # Extract features for each window of data
    data_features = []
    data_features_dict = {}
    for i in range(0, len(data_shuffled), 500):
        window = data_shuffled[i : i + 500]
        window_features = {}
        for column in window.columns:
            column_data = window[column]
            window_features[column] = calculate_features(column_data)
        data_features.append(window_features)

    # This allows us to associate the features dictionary with each collection index {index : features dict}
    for i in range(len(data_features)):
        data_features_dict[i] = data_features[i]
